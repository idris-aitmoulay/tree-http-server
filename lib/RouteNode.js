...
RouteNode.prototype.findHandler = function (path, method) {
  ...
  if (path === ROOT_SEGMENT || path === EMPTY_STRING) {
...
RouteNode.prototype.computeRoutes = function (basePath, computedRoutesMap = {}) {
  const { handlers, children } = this;
  computedRoutesMap[basePath] = Object.keys(handlers);
  for (const childKey of Object.keys(children)) {
    const childPath = basePath + ROOT_SEGMENT + childKey;
    this.children[childKey].computeRoutes(childPath, computedRoutesMap)
  }
}

RouteNode.prototype.getRoutes =  function () {
  let treePath = {};
  const { handlers, children } = this;
  treePath[ROOT_SEGMENT] = Object.keys(handlers)
  for (const childKey of Object.keys(children)) {
    const childPath = ROOT_SEGMENT + childKey;
    this.children[childKey].computeRoutes(childPath, treePath)
  }
  return treePath;
}
